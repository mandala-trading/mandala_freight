...
class PageSetting < ApplicationRecord
  MODULE_NAMES = %w[master_countries master_currencies master_ports master_buyers master_container_details
                    master_shipping_lines master_freight_items master_units].freeze
  MODULE_CLASSES = %w[Country Currency Port Buyer ContainerDetail ShippingLine FreightItem Unit].freeze
